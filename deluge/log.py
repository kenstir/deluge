...
if 'dev' in common.get_version():
    DEFAULT_LOGGING_FORMAT = '%%(asctime)s.%%(msecs)03.0f [%%(process)-7d.%%(thread)d)][%%(levelname)-8s][%%(name)-%ds %%(filename)-17s:%%(lineno)-4d] %%(message)s'
else:
    DEFAULT_LOGGING_FORMAT = '%%(asctime)s [%%(process)-7d.%%(thread)][%%(levelname)-8s][%%(name)-%ds:%%(lineno)-4d] %%(message)s'
